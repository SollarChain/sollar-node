 ...
 class SollarMasterTokenContract extends SollarTokenContract {
     ...
     init(emission, mintable=true) {
        ...
        this._alreadyValidate = new BlockchainArray('alreadyValidate');
     ...
     getNodesCountOnline() {
        ...
        for (const address of this._whiteListWallets) {
            const wallet = this._whiteList[address];
            if (this._checkWalletStatus(wallet)) {
                counts++;
            }
        }
    ...
    _calculateValidators() {
        ...
        for (const address of this._whiteListWallets) {
            ...
            if (this._checkWalletStatus(wallet)) {
                ...
                const isAlreadyValidated = this._alreadyValidate.includes(address);
    ...
    checkIsNodeCanValidate(address = this._getSender(), onlineNodes) {
        ...
        // console.log('checkWalletStatus', checkWalletStatus);
        if (!checkWalletStatus) {
            return false;
        }

        const nodesOnline = this.getNodesCountOnline();
        // console.log('nodesOnline', nodesOnline);
        if (nodesOnline < 2) {
            return true;
        } else {
            const [counts, validated] = this._calculateValidators();
            // console.log('check validators count CALL', counts, validated);
            // this._alreadyValidate.forEach(data => {
            ...
            if (validated >= counts) {
                ...
                const isAlreadyValidated = this._alreadyValidate.includes(address);
                // console.log('isAlreadyValidated', isAlreadyValidated);
                return !isAlreadyValidated;
            }
        }
    }

    _checkStateIsNodeCanValidate(address = this._getSender()) {
        const wallet = this._whiteList[address];
        ...
        if (nodesOnline < 2) {
            ...
            const [counts, validated] = this._calculateValidators();
            // console.log('check validators count DEPLOY', counts, validated);
            // this._alreadyValidate.forEach(data => {
            //     console.log('validated', data);
            // })
            if (validated >= counts) {
                // this._alreadyValidate.applyArray([]);
                this._alreadyValidate = new BlockchainArray('alreadyValidate_' + Date.now().toString());
                return true;
            } else {
                const isAlreadyValidated = this._alreadyValidate.includes(address);
                // console.log('isAlreadyValidated', isAlreadyValidated);
    ...
    _changeStateToNodeIsValidated(nodeAddress = this._getSender()) {
        ...
        if (!this._alreadyValidate.includes(nodeAddress)) {
            this._alreadyValidate.push(nodeAddress);
        }
        // console.log('_alreadyValidate new', this._alreadyValidate.length);
    ...
    calculateBlockFee(state) {
        ...
        const blockEmission = this._changeStateToNodeIsValidated(reciever, wallet.owner);
    ...
    // Adding node to white list
     addNodeToWhiteList(ownerWallet, nodeWallet = this._getSender()) {
        console.log('Adding node to white list, owner:', ownerWallet, 'node:', nodeWallet);
        ...
        console.log('transfer', ownerWallet, this._contract['wallet'], validatorFee);
        this._wallets.transfer(ownerWallet, this._contract['wallet'], validatorFee);

        console.log('event', 'SOL', ownerWallet, this._contract['wallet'], validatorFee, 0, Date.now());

        this._TransferFeeEvent.emit('SOL', ownerWallet, this._contract['wallet'], validatorFee, 0, Date.now()); // TODO add fee
