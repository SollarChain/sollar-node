...
class starwaveProtocol {
        ...
        if(typeof that.blockchain !== 'undefined') {
            if(messageBusAddress === this.getAddress()) { //message to yourself
                ...
                if(!socket) {  //there is no such connected socket
                    ...
                    const routeIsNotComplete = !this.routeIsComplete(message);
                    
                    if(routeIsNotComplete) {
     ...
     */
    sendMessage(message) {
        const isSendMessageToPeers = !this.sendMessageToPeer(message.reciver, message);
        if(isSendMessageToPeers) {       //it was not possible to send directly, there is no directly connected peer, we make a newsletter to everyone
